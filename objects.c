/******************************************************************************
	...
	Change Control:													DDMMYYYY
		...
		Michael Still		Fixed bugs, added write object,			17062000
...
void adddictitem(dictionary *victim, char *name, int valueType, ...){
...
void writeObject(pdf *output, object *dumpTarget){
	...
	if(dumpTarget->number != -1){
		...
		if((dumpTarget->streamLength > 0) && (dumpTarget->stream != NULL)){
			adddictitem(dumpTarget->dict, "Length", gIntValue, dumpTarget->streamLength);
		}
