...
/* a byte range locking function - return 0 on success
   ...
   through tdb_open(). */
static int
tdb_brlock (TDB_CONTEXT * tdb, tdb_off offset,
...
static int
tdb_lock (TDB_CONTEXT * tdb, int list, int ltype)
{
...
static void
tdb_unlock (TDB_CONTEXT * tdb, int list, int ltype)
{
  if (tdb->flags & TDB_NOLOCK)
    return;
...
*/
static int
tdb_oob (TDB_CONTEXT * tdb, tdb_off len, int probe)
{
  struct stat st;
  if (len <= tdb->map_size)
    return 0;
...
static int
tdb_write (TDB_CONTEXT * tdb, tdb_off off, void *buf, tdb_len len)
{
...
static int
tdb_read (TDB_CONTEXT * tdb, tdb_off off, void *buf, tdb_len len, int cv)
{
  if (tdb_oob (tdb, off + len, 0) != 0)
    return -1;
...
static char *
tdb_alloc_read (TDB_CONTEXT * tdb, tdb_off offset, tdb_len len)
{
  char *buf;
...
static int
ofs_read (TDB_CONTEXT * tdb, tdb_off offset, tdb_off * d)
{
  return tdb_read (tdb, offset, (char *) d, sizeof (*d), DOCONV ());
}
static int
ofs_write (TDB_CONTEXT * tdb, tdb_off offset, tdb_off * d)
{
  tdb_off off = *d;
...
/* read/write a record */
static int
rec_read (TDB_CONTEXT * tdb, tdb_off offset, struct list_struct *rec)
{
...
static int
rec_write (TDB_CONTEXT * tdb, tdb_off offset, struct list_struct *rec)
{
...
static int
rec_free_read (TDB_CONTEXT * tdb, tdb_off off, struct list_struct *rec)
{
...
static int
update_tailer (TDB_CONTEXT * tdb, tdb_off offset,
	       const struct list_struct *rec)
{
  ...
  return ofs_write (tdb, offset + totalsize - sizeof (tdb_off), &totalsize);
}

static tdb_off
tdb_dump_record (TDB_CONTEXT * tdb, tdb_off offset)
{
  ...
  printf
    (" rec: offset=%u next=%d rec_len=%d key_len=%d data_len=%d full_hash=0x%x magic=0x%x\n",
     offset, rec.next, rec.rec_len, rec.key_len, rec.data_len, rec.full_hash,
     rec.magic);
...
static void
tdb_dump_chain (TDB_CONTEXT * tdb, int i)
{
  tdb_off rec_ptr, top;
...
static int
remove_from_freelist (TDB_CONTEXT * tdb, tdb_off off, tdb_off next)
{
  tdb_off last_ptr, i;
...
/* Add an element into the freelist. Merge adjacent records if
   neccessary. */
static int
tdb_free (TDB_CONTEXT * tdb, tdb_off offset, struct list_struct *rec)
{
  tdb_off right, left;
...
static int
expand_file (TDB_CONTEXT * tdb, tdb_off size, tdb_off addition)
{
  char buf[1024];
  ...
  if (lseek (tdb->fd, (size + addition) - 1, SEEK_SET) !=
      (size + addition) - 1 || write (tdb->fd, &b, 1) != 1)
    {
#endif
...
static int
tdb_expand (TDB_CONTEXT * tdb, tdb_off size)
{
  struct list_struct rec;
  tdb_off offset;
 ...
 */
static tdb_off
tdb_allocate (TDB_CONTEXT * tdb, tdb_len length, struct list_struct *rec)
{
...
static int
tdb_new_database (TDB_CONTEXT * tdb, int hash_size)
{
...
/* Returns 0 on fail.  On success, return offset of record, and fills
   in rec */
static tdb_off
tdb_find (TDB_CONTEXT * tdb, TDB_DATA key, u32 hash, struct list_struct *r)
{
  tdb_off rec_ptr;
  ...
  while (rec_ptr)
      ...
      if (!TDB_DEAD (r) && hash == r->full_hash && key.dsize == r->key_len)
	{
	  ...
	  k = tdb_alloc_read (tdb, rec_ptr + sizeof (*r), r->key_len);
	  if (!k)
	    return 0;
...
static int
tdb_keylocked (TDB_CONTEXT * tdb, u32 hash)
{
  u32 i;
  if (!tdb->lockedkeys)
    return 1;
...
static tdb_off
tdb_find_lock (TDB_CONTEXT * tdb, TDB_DATA key, int locktype,
	       struct list_struct *rec)
{
  u32 hash, rec_ptr;
...
*/
static int
tdb_update (TDB_CONTEXT * tdb, TDB_DATA key, TDB_DATA dbuf)
{
  struct list_struct rec;
  tdb_off rec_ptr;
  int ret = -1;
...
static int
lock_record (TDB_CONTEXT * tdb, tdb_off off)
{
...
static int
write_lock_record (TDB_CONTEXT * tdb, tdb_off off)
{
...
static int
write_unlock_record (TDB_CONTEXT * tdb, tdb_off off)
{
...
static int
unlock_record (TDB_CONTEXT * tdb, tdb_off off)
{
  struct tdb_traverse_lock *i;
  u32 count = 0;
...
static int
do_delete (TDB_CONTEXT * tdb, tdb_off rec_ptr, struct list_struct *rec)
{
  tdb_off last_ptr, i;
...
static int
tdb_next_lock (TDB_CONTEXT * tdb, struct tdb_traverse_lock *tlock,
	       struct list_struct *rec)
{
  ...
  for (; tlock->hash < tdb->header.hash_size; tlock->hash++)
	...
	return -1;
      ...
      if (!tlock->off)
	{
	  if (ofs_read (tdb, TDB_HASH_TOP (tlock->hash), &tlock->off) == -1)
	    goto fail;
	}
      else
	{
...
/* traverse the entire database - calling fn(tdb, key, data) on each element.
  ...
  */
int
tdb_traverse (TDB_CONTEXT * tdb, tdb_traverse_func fn, void *state)
{
  TDB_DATA key, dbuf;
  struct list_struct rec;
...
tdb_firstkey (TDB_CONTEXT * tdb)
{
  ...
  key.dsize = rec.key_len;
  key.dptr =
    tdb_alloc_read (tdb, tdb->travlocks.off + sizeof (rec), key.dsize);
...
TDB_DATA
tdb_nextkey (TDB_CONTEXT * tdb, TDB_DATA oldkey)
{
  u32 oldhash;
...
*/
int
tdb_store (TDB_CONTEXT * tdb, TDB_DATA key, TDB_DATA dbuf, int flag)
{
  struct list_struct rec;
  u32 hash;
  tdb_off rec_ptr;
...
static int
tdb_already_open (dev_t device, ino_t ino)
{
  TDB_CONTEXT *i;
...
TDB_CONTEXT *
tdb_open (const char *name, int hash_size, int tdb_flags,
...
TDB_CONTEXT *
tdb_open_ex (const char *name, int hash_size, int tdb_flags,
	     int open_flags, mode_t mode, tdb_log_func log_fn)
{
  TDB_CONTEXT *tdb;
  struct stat st;
  int rev = 0, locked;
...
int
tdb_lockkeys (TDB_CONTEXT * tdb, u32 number, TDB_DATA keys[])
{
  u32 i, j, hash;
  ...
  for (i = 0; i < number; i++)
    {
      ...
      for (j = 0; j < i && BUCKET (tdb->lockedkeys[j + 1]) < BUCKET (hash);
	   j++);
...
void
tdb_unlockkeys (TDB_CONTEXT * tdb)
{
  u32 i;
  for (i = 0; i < tdb->lockedkeys[0]; i++)
...
int
tdb_chainlock (TDB_CONTEXT * tdb, TDB_DATA key)
{
  return tdb_lock (tdb, BUCKET (tdb_hash (&key)), F_WRLCK);
}

void
tdb_chainunlock (TDB_CONTEXT * tdb, TDB_DATA key)
{
...
void
tdb_logging_function (TDB_CONTEXT * tdb,
		      void (*fn) (TDB_CONTEXT *, int, const char *, ...))
{
  tdb->log_fn = fn;
}
